...
TEST_F(IssueTrackerViewTest, CreateIssue_ForwardsInputToController) {
  EXPECT_CALL(mockController,
      createIssue("Title", "Description", "User123"))
      .WillOnce(Return(Issue()));

  runWithInput("Title\nDescription\nUser123\n",
      &IssueTrackerView::createIssue);
}
...
TEST_F(IssueTrackerViewTest, DeleteIssue_PrintsSuccessMessage) {
  ...
  const std::string output = runWithInput("5\n",
...
TEST_F(IssueTrackerViewTest, AddComment_PrintsSuccessMessage) {
  ...
  const std::string output = runWithInput("9\nauthor\nGreat job\n",
...
TEST_F(IssueTrackerViewTest, UpdateIssue_PrintsSuccessMessage) {
  ...
  const std::string output = runWithInput("7\ntitle\nNew title\n",
...
TEST_F(IssueTrackerViewTest, UnassignUser_PrintsSuccessMessage) {
  ...
  const std::string output = runWithInput("11\n",
...
TEST_F(IssueTrackerViewTest, UpdateUser_PrintsMenuOptions) {
  const std::string output = runWithInput("3\n",
      &IssueTrackerView::updateUser);

  EXPECT_THAT(output, HasSubstr("1: Assigned Issues"));
}

TEST_F(IssueTrackerViewTest, Run_ShowsMenuAndExits) {
  const std::string output = runWithInput("0\n", &IssueTrackerView::run);
