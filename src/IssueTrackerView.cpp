...
// ---------- local helpers (file-internal) ----------
namespace {

/**
 * Format epoch milliseconds to "YYYY-MM-DD HH:MM" in local time.
 * Returns "unknown" if ms <= 0.
 */
template <typename IntT>
std::string fmtEpochMs(IntT ms) {
  if (ms <= 0) return "unknown";
  std::time_t secs = static_cast<std::time_t>(ms / 1000);
  std::tm tm {};
  // POSIX-safe; on Windows use localtime_s(&tm, &secs)
  localtime_r(&secs, &tm);
...
/** Print a single comment in a compact, readable line. */
void printCommentLine(const Comment& c) {
  std::cout << "  - [" << c.getId() << "] "
            << c.getAuthor() << " @ "
            << fmtEpochMs(c.getTimeStamp()) << "\n"
            << "    " << c.getText() << "\n";
}
...
void IssueTrackerView::createIssue() {
  ...
  std::cout << "Issue assigned to user: " << assignedTo << "\n";

  Issue issue = controller->createIssue(title, desc, assignedTo);
  (void)issue;  // currently unused in view
}
...
void IssueTrackerView::listIssues() {
  ...
  for (const auto& issue : issues) {
    ...
    std::cout << "Created: " << fmtEpochMs(issue.getTimestamp()) << "\n";
    ...
    if (issue.hasDescriptionComment()) {
      ...
      if (desc) {
        std::cout << "Description (" << fmtEpochMs(desc->getTimeStamp())
                  << "): " << desc->getText() << "\n";
      } else {
...
void IssueTrackerView::findIssuesByUser() {
  ...
  if (issues.empty()) {
    std::cout << "No issues for user.\n";
    return;
  }
  for (const auto& i : issues) {
    std::cout << "ID: " << i.getId()
              << " | Title: " << i.getTitle()
              << " | Created: " << fmtEpochMs(i.getTimestamp()) << "\n";
  }
}
...
void IssueTrackerView::updateUser() {
  ...
  std::cout << "1: User name\n";
  std::cout << "2: User Role\n";
  std::cin >> choice;
  ...
  // consume newline after last >> read
...
int IssueTrackerView::getissueId() {
  ...
  std::cout << "\n--- All Issues ---\n";
  for (const auto& issue : issues) {
    std::cout << idx << "). ID: " << issue.getId()
              << " -- Title: " << issue.getTitle() << "\n";
    ++idx;
    issueids.push_back(issue.getId());
  }

  int userinput = getvalidInt(static_cast<int>(issueids.size()));
  return issueids[userinput - 1];
}

int IssueTrackerView::getvalidInt(int bound) {
  if (bound < 1) {
    std::cerr << "Error: The validation bound must be 1 or greater.\n";
    return -1;
  }

  int selection;
  while (true) {
    std::cout << "Please enter an integer between 1 and "
              << bound << ": ";
...
void IssueTrackerView::displayIssue(int id) {
  ...
  std::cout << "Created: " << fmtEpochMs(iss.getTimestamp()) << "\n";
  std::cout << "Comments: " << iss.getCommentIds().size() << "\n";

  // Print description if linked
  if (iss.hasDescriptionComment()) {
    if (const Comment* d =
            iss.findCommentById(iss.getDescriptionCommentId())) {
      std::cout << "Description (" << fmtEpochMs(d->getTimeStamp())
                << "): " << d->getText() << "\n";
    }
  }

  // Print all comments with timestamps
...
void IssueTrackerView::addComIssue() {
  ...
  std::cout << "Comment text add here\n";
